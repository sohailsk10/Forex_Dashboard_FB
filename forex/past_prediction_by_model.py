    ...
    try:
        for d in range(delta.days):
            ...
            for m in range(1440 // interval_int):
                minute += interval_int
                _tmp = _from_date + timedelta(days=d, minutes=minute)
                _day = _tmp.strftime('%a')
                print('DATE TIME', _from_date + timedelta(days=d, minutes=minute), _from_date, _from_date + timedelta(days=d, minutes=minute), interval_int)

                if _day != 'Sat' and _day != 'Sun':
                    og_df = get_data_mt5(
                    ...
                    if og_df is not None and len(og_df) != 0:
                        ...
                        if not high_df_forecast.empty and not low_df_forecast.empty:
                            ...
                            ReportHistoryPrediction.objects.create(report_status=_rs,
